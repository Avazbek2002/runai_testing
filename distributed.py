...
def load_and_prepare_data(tokenizer, local_rank):
    ...
    full_dataset = load_dataset("uzinfocom/AT_atamalar")
    # Select the first 15000 rows
    dataset = full_dataset['train'].select(range(10000))
    ...
    def prompt_builder(row):
        return {"text": f'Definition of {row["Header"]}: {row["Content"]}'}
    ...
    train_dataset = train_dataset.remove_columns(['Header', 'Content'])
    test_dataset = test_dataset.remove_columns(['Header', 'Content'])
